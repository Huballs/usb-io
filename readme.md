...
## conan
```
conan install . --profile:a=msvcd  -s build_type=Debug --build=missing --output-folder=cmake-build-debug-msvc
```
...
## conan
```
conan install . --profile:a=gccd  -s build_type=Debug --build=missing --output-folder=cmake-build-debug
```
